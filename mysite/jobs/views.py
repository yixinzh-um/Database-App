...
from jobs.models import Job,Comment, Fav, Employer,Location
from jobs.owner import OwnerListView,OwnerDetailView,OwnerCreateView,OwnerUpdateView,OwnerDeleteView
from django.contrib.auth.mixins import LoginRequiredMixin
from jobs.forms import CreateForm,CommentForm,EmployerForm
from django.shortcuts import render, redirect, get_object_or_404
from django.http import HttpResponse
from django.contrib.humanize.templatetags.humanize import naturaltime
from jobs.utils import dump_queries
...
class JobListView(OwnerListView):
    ...
    def get(self, request) :
        ...
        if strval :
            ...
            job_list = Job.objects.filter(query).select_related().order_by('-updated_at')[:100]
        else :
            job_list = Job.objects.all().order_by('-updated_at')[:100]
...
class EmployerDetailView(OwnerDetailView):
    model = Employer
    template_name = "employers/employer_detail.html"
    def get(self, request, pk) :
        x = Employer.objects.get(id=pk)
        j = list(Job.objects.filter(employer=pk).select_related().order_by('-updated_at'))
        ...
        if request.user.is_authenticated:
            ...
            # favorites = [2, 4, ...] using list comprehension
            favorites = [ row['id'] for row in rows ]
        context = { 'employer' : x,'job_list':j,'favorites': favorites}
        return render(request, self.template_name, context)
...
class CommentCreateView(LoginRequiredMixin, View):
    def post(self, request, pk) :
        f = get_object_or_404(Employer, id=pk)
